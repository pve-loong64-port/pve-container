...
sub vmstatus {
    ...
    my $list =
        $opt_vmid ? { $opt_vmid => { type => 'lxc', vmid => int($opt_vmid) } } : config_list();
...
sub make_seccomp_config {
    ...
    my $rule_data = file_get_contents("$LXC_CONFIG_PATH/common.seccomp") . $extra_rules;
...
sub make_apparmor_config {
    ...
    if (PVE::LXC::Config->has_lxc_entry($conf, 'lxc.apparmor.profile')) {
        if (length(my $used = join(', ', @profile_uses))) {
            warn
                "explicitly configured lxc.apparmor.profile overrides the following settings: $used\n";
        }
        return '';
    }
...
sub update_lxc_config {
    ...
    PVE::LXC::Config->foreach_passthrough_device(
        ...
        sub {
            ...
            $raw .=
                "lxc.cgroup2.devices.allow = $device_type_char $major:$minor $allow_perms\n";
...
sub destroy_lxc_container {
    ...
    PVE::LXC::Config->foreach_volume_full($conf->{pending}, { include_unused => 1 },
        $remove_volume);
...
sub update_net {
    ...
    if (my $oldnetcfg = $conf->{$opt}) {
        ...
        if (
            safe_string_ne($oldnet->{hwaddr}, $newnet->{hwaddr})
            ...
            if (
                ...
                if (my $err = $@) {
                    ...
                    warn "restoring old network due to hot-plug failure for $veth failed - $@"
                        if $@;
                    die "$err\n";
                }
...
sub hotplug_net {
    ...
    if ($have_sdn) {
        PVE::Network::SDN::Zones::veth_create(
            $veth, $vethpeer, $newnet->{bridge}, $newnet->{hwaddr},
        );
    } else {
    ...
    $cmd =
        ['lxc-attach', '-n', $vmid, '-s', 'NETWORK', '--', '/sbin/ip', 'link', 'set', $eth, 'up'];
    PVE::Tools::run_command($cmd);
    ...
    my $change_ip_config = sub {
        ...
        if ($change_gw) {
            if ($newgw) {
                eval {
                    if (
                        $is_real_ip && !PVE::Network::is_ip_in_cidr($newgw, $newip, $ipversion)
                    ) {
        ...
        if ($change_ip && $oldip && $oldip !~ /^(?:auto|dhcp)$/) {
            ...
            eval {
                if ($ipversion == 4) {
                    &$nscmd(
                        { outfunc => sub { $promote = int(shift) } },
                        'cat',
                        "/proc/sys/net/ipv4/conf/$eth/promote_secondaries",
                    );
...
sub umount_all {
    ...
    PVE::LXC::Config->foreach_volume_full(
        ...
        sub {
            ...
            eval { PVE::Tools::run_command(['umount', '-d', $mount_path]); };
            if (my $err = $@) {
                if ($noerr) {
                    $res = 0;
                    warn $err;
...
sub __bindmount_verify {
    ...
    if ($parentfd) {
        ...
        # mounted path.
        $destdh = PVE::Tools::openat(
            fileno($parentfd),
            $last_dir,
            PVE::Tools::O_PATH | O_NOFOLLOW | O_DIRECTORY,
        );
...
sub __mount_prepare_rootdir {
    ...
    my ($mpfd, $parentfd, $last_dir) =
        walk_tree_nofollow($rootdir, $mount, 1, $root_uid, $root_gid);
...
sub mountpoint_mount {
    ...
    return __mountpoint_mount(
        $mountpoint, $rootdir, $storage_cfg, $snapname, $root_uid, $root_gid, undef,
    );
}

sub mountpoint_stage {
    my ($mountpoint, $stage_dir, $storage_cfg, $snapname, $root_uid, $root_gid) = @_;
    my ($path, $loop, $dev) = __mountpoint_mount(
        $mountpoint, $stage_dir, $storage_cfg, $snapname, $root_uid, $root_gid, 1,
    );
...
sub __mountpoint_mount {
    ...
    if ($storage) {
        ...
        bindmount($volid, $parentfd, $last_dir // $rootdir, $mount_path, $readonly, @extra_opts)
            if $mount_path;
        ...
        PVE::LXC::Config->foreach_volume(
            ...
            sub {
                ...
                if ($storage && ($volid =~ $NEW_DISK_RE)) {
                    ...
                    my $needs_chown = 0;
                    ($volid, $needs_chown) =
                        alloc_disk($storecfg, $vmid, $storage, $size_kb, $root_uid, $root_gid);
...
sub parse_id_maps {
    ...
    if (!@$id_map && $conf->{unprivileged}) {
        # Should we read them from /etc/subuid?
        $id_map = [['u', '0', '100000', '65536'], ['g', '0', '100000', '65536']];
...
my sub monitor_state_change($$) {
    ...
    while (1) {
        ...
        if ($value eq PVE::LXC::Monitor::STATE_STARTING) {
            ...
            $value eq PVE::LXC::Monitor::STATE_ABORTING
            ...
            warn "unexpected message from monitor socket - " . "type: '$type' - value: '$value'\n";
        }
    }
}
my sub monitor_start($$) {
    ...
    my $success =
        eval { PVE::Tools::run_with_timeout(10, \&monitor_state_change, $monitor_socket, $vmid) };
    if (my $err = $@) {
...
sub vm_start {
    ...
    if ($skiplock) {
        open(my $fh, '>', $skiplock_flag_fn)
            || die "failed to open $skiplock_flag_fn for writing: $!\n";
        close($fh);
    }
...
my $copy_volume = sub {
    ...
    foreach my $mount (reverse @mounted) {
        eval {
            run_command(['/bin/umount', $mount], errfunc => sub { });
        };
...
sub copy_volume {
    ...
    eval {
        ...
        my $needs_chown;
        ($new_volid, $needs_chown) =
            alloc_disk($storage_cfg, $vmid, $storage, $mp->{size} / 1024, $root_uid, $root_gid);
        if ($needs_chown) {
...
sub create_ifaces_ipams_ips {
    ...
    for my $opt (keys %$conf) {
        ...
        PVE::Network::SDN::Vnets::add_next_free_cidr(
            $net->{bridge}, $conf->{hostname}, $net->{hwaddr}, $vmid, undef, 1,
        );
    }
}
